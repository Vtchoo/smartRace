			...
			for (var m = 0 ; m < offs ; m++){
				for (var n = 0 ; n < population ; n++){
					...
					if ( count == population - m){
						...
						cars[n].paintRGB = [(sqrt((sq(cars[parent1].paintRGB[0])+sq(cars[parent2].paintRGB[0]))/2)+(Math.round(Math.random()) * 2 - 1)*nnMutationRate*255)%255, 
								    (sqrt((sq(cars[parent1].paintRGB[1])+sq(cars[parent2].paintRGB[1]))/2)+(Math.round(Math.random()) * 2 - 1)*nnMutationRate*255)%255, 
								    (sqrt((sq(cars[parent1].paintRGB[2])+sq(cars[parent2].paintRGB[2]))/2)+(Math.round(Math.random()) * 2 - 1)*nnMutationRate*255)%255]
